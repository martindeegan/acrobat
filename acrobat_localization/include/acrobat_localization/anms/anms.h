...
std::vector<cv::KeyPoint> GridFAST(cv::Mat Image, int numRetPoints, int gridRows, int gridCols) {
    std::vector<cv::KeyPoint>    kp;
    cv::Ptr<cv::FeatureDetector> featdetectorK = new cv::GridAdaptedFeatureDetector(
        cv::FeatureDetector::create("FAST"), numRetPoints, gridRows, gridCols);
...
std::vector<cv::KeyPoint> Sdc(std::vector<cv::KeyPoint> keyPoints,
    ...
    while (!complete) {
        ...
        if (radius == prevradius ||
            low > high) {       // needed to reassure the same radius is not repeated again
        ...
        for (unsigned int i = 0; i < keyPoints.size(); ++i) {
            ...
            if (coveredVec[row][col] == false) { // if the cell is not covered
                ...
                int rowMax = ((row + floor(radius / c)) <= numCellRows) ? (row + floor(radius / c))
                                                                        : numCellRows;
                int colMin = ((col - floor(radius / c)) >= 0) ? (col - floor(radius / c)) : 0;
                int colMax = ((col + floor(radius / c)) <= numCellCols) ? (col + floor(radius / c))
                                                                        : numCellCols;
...
std::vector<cv::KeyPoint> KdTree(std::vector<cv::KeyPoint> keyPoints,
    ...
    long long exp2 =
        ((long long)4 * cols + (long long)4 * numRetPoints + (long long)4 * rows * numRetPoints +
    ...
    int high =
        (sol1 > sol2) ? sol1 : sol2; // binary search range initialization with positive solution
    ...
    using my_kd_tree_t =
        nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<int, PointCloud<int>>,
                                            PointCloud<int>,
                                            2>;
...
std::vector<cv::KeyPoint> RangeTree(std::vector<cv::KeyPoint> keyPoints,
    ...
    rangetree<u16, u16> treeANMS(keyPoints.size(),
    ...
    while (!complete) {
        ...
        for (unsigned int i = 0; i < keyPoints.size(); ++i) {
            if (Included[i] == true) {
                ...
                int maxx = keyPoints[i].pt.x + width; // defining square boundaries around the point
...
std::vector<cv::KeyPoint> Ssc(std::vector<cv::KeyPoint> keyPoints,
                              ...
                              int                       rows) {
    // several temp expression variables to simplify solution equation
    const int       exp1 = rows + cols + 2 * numRetPoints;
    const long long exp2 =
        ((long long)4 * cols + (long long)4 * numRetPoints + (long long)4 * rows * numRetPoints +
         (long long)rows * rows + (long long)cols * cols - (long long)2 * rows * cols +
         (long long)4 * rows * cols * numRetPoints);
    const double exp3 = sqrt(exp2);
    const double exp4 = numRetPoints - 1;
    ...
    std::vector<int> ResultVec;
    bool             complete = false;
    const size_t     K        = numRetPoints;
    const size_t     Kmin     = round(K - (K * tolerance));
    const size_t     Kmax     = round(K + (K * tolerance));
        ...
        for (size_t i = 0; i < keyPoints.size(); ++i) {
            ...
            if (coveredVec[row][col] == false) { // if the cell is not covered
                ...
                int rowMin = ((row - floor(width / c)) >= 0)
                                 ? (row - floor(width / c))
                                 : 0; // get range which current radius is covering
                int rowMax = ((row + floor(width / c)) <= numCellRows) ? (row + floor(width / c))
                                                                       : numCellRows;
                int colMin = ((col - floor(width / c)) >= 0) ? (col - floor(width / c)) : 0;
                int colMax = ((col + floor(width / c)) <= numCellCols) ? (col + floor(width / c))
                                                                       : numCellCols;
                for (int rowToCov = rowMin; rowToCov <= rowMax; ++rowToCov) {
                    for (int colToCov = colMin; colToCov <= colMax; ++colToCov) {
    ...
    // retrieve final keypoints
    std::vector<cv::KeyPoint> kp;
    for (size_t i = 0; i < ResultVec.size(); i++) kp.push_back(keyPoints[ResultVec[i]]);

    return kp;
}

void VisualizeAll(cv::Mat Image, std::vector<cv::KeyPoint> keyPoints, std::string figureTitle) {
    cv::Mat resultImg;
